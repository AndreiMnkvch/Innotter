...
# def page_statistics_service(page: Page) -> JsonResponse:
#     posts_count = Post.objects.filter(page=page.pk).count()
#     followers_count = page.followers.count()
#     likes_count = Post.objects.filter(page=page.pk).aggregate(Sum('likes'))
